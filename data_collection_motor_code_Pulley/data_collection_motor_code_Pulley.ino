 ...
 * November 15, 2019
 * Crated new file for pulley -- motor must turn oppostite direction for correct movement. 
...
static int frontprotothread(struct pt*pt){   //setting up protothread for user input "BACK"
  PT_BEGIN(pt);
...
void goback(){  //function for movint to the front
...
void ISR_backward(){   //forward interrupt function
if(digitalRead(interruptpin1)==HIGH){ //if limit switch pressed
  allowreverse=false;  //forward motion not allowed
    
}
else{
  allowreverse=true;  //forward motion allowed
}
}

void ISR_forward(){ //Backward interrupt function
if(digitalRead(interruptpin2)==HIGH){  //if limit switch pressed
  allowforward=false; //backward motion not allowed 
}
else{
  allowforward=true;   //backward motion allowed
}
}

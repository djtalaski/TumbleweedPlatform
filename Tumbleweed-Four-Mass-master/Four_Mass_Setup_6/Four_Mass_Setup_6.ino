...
#define SP_angvel                200.00f  // deg/s  (60 = 10rpm)  (90 = 15rpm)
...
#define CONE_WIDTH           20      // cone angle from vertical
#define sampleSize           2
...
void loop()
{
  ...
  //    if ((angular_data.velocity <= SP_angvel))  //
  //
  //   {
  //    // Acceleration
...
void amplitude_from_angle(pidInfo &pid1, pidInfo &pid2, pidInfo &pid3, pidInfo &pid4, angleData &data)
{
  ...
  float dvel = data.velocity / SP_angvel;
  float secnd = MAX_AMPLITUDE * (1 - (dvel));
  float aza = -secnd + SP_mean;
  float bzb = +secnd + SP_mean;

  aza = abs(aza);
  bzb = abs(bzb);
  //Serial.print("Vel=");
  // Serial.print(data.velocity);
  // Serial.print("Velratio=");
  //  Serial.print(dvel);
  //  Serial.print("Addendum=");
  //   Serial.print(secnd);
  //   Serial.print("Ex1=");
  //    Serial.print(aza);
  //     Serial.print("Ex2=");
  //  Serial.println(bzb);
  // int cc=SP_mean + MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
  //  int dd=SP_mean - MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
  ...
  if (abs(data.velocity) <= SP_angvel) // When current angvel less than desired angvel
  {
    ...
    if ((270 + CONE_WIDTH) > data.curr_angle && data.curr_angle > (270 - CONE_WIDTH))
    {
      ...
      pid1.sp = bzb;//SP_mean + MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid2.sp = aza;//SP_mean - MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid3.sp = aza;//SP_mean - MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel)); // assume pid1 is left motor and up
      pid4.sp = bzb;//SP_mean + MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid1.i = 0;
      pid1.pe = 0;
      pid2.i = 0;
      pid2.pe = 0;
    ...
    if ((180 + CONE_WIDTH) > data.curr_angle && data.curr_angle > (180 - CONE_WIDTH))
    {
      ...
      //        pid1.ki=0.0001;
      //        pid1.kd=0.0001;
      pid1.sp = aza;//SP_mean - MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid2.sp = bzb;//SP_mean + MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid3.sp = aza;//SP_mean - MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel)); // assume pid1 is left motor and up
      pid4.sp = bzb;//SP_mean + MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid1.i = 0;
      pid1.pe = 0;
    ...
    if ((90 + CONE_WIDTH) > data.curr_angle && data.curr_angle > (90 - CONE_WIDTH))
    {
      ...
      pid1.sp = aza; //SP_mean - MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid2.sp = bzb; //SP_mean + MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid3.sp = bzb; //SP_mean + MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel)); // assume pid1 is left motor and up
      pid4.sp = aza; //SP_mean - MAX_AMPLITUDE*(1 - (data.velocity/SP_angvel));
      pid1.i = 0;
      pid1.pe = 0;
      pid2.i = 0;
      pid2.pe = 0;
